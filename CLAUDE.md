...
AFPSCharacter         [复制]       血量/体力、移动、死亡/复活、持有武器、移动状态、背包组件
AFPSWeapon            [复制 Actor]  武器射击、服务端权威碰撞检测、弹药。网格体由蓝图子类创建
UFPSInventoryComponent[挂 Character] 背包：服务端权威增删/使用，COND_OwnerOnly 复制给本人
UFPSItemDef           [DataAsset]   道具数据定义（多态子类，所有端本地各持一份，不复制）
AFPSPickup            [复制 Actor]  地上拾取物，空 Root + Sphere 触发器。网格体由蓝图子类创建
```
...
| `IsFiring()` | `CurrentWeapon->IsFiring()` | 服务端复制到所有端 |
| `GetWeapon()` | `AFPSWeapon*` | 当前持有的武器 |
| `GetArmPitch()` | `float` | 复制到所有端的瞄准俯仰角 |

## 输入系统（纯 Enhanced Input，Axis2D 架构）

C++ 不再动态创建 InputAction / IMC / Modifier。所有资产由蓝图填到 `BP_FPSPlayer` 的 Input 分类下的 UPROPERTY：`InputMappingContext / InputMove / InputLook / InputJump / InputRun / InputAim / InputFire / InputReload / InputInteract / InputInventory`。
...
| F | InputInteract | Boolean | `Interact` | Started→拾取当前范围内 Pickup（转发 ServerTryPickup） |
...
`AFPSWeapon` 持有弹药状态，**逻辑全在 C++ 服务端权威**，蓝图子类只在 default 里填三个数值。

### 配置（蓝图子类 EditDefaultsOnly）
| 属性 | 默认 | 含义 |
|---|---|---|
| `MagSize` | 30 | 弹夹容量 |
| `InitialReserveAmmo` | 120 | 开局/复活时弹夹外备弹 |
...
| `ReserveAmmo` | `ReplicatedUsing=OnRep_Ammo` | 备弹数 |
| `bIsReloading` | `ReplicatedUsing=OnRep_IsReloading` | 换弹中（禁火 + 触发换弹动画） |

### 蓝图可调用 getter（所有端）
`GetCurrentAmmo()` / `GetReserveAmmo()` / `GetMagSize()` / `IsReloading()` / `CanReload()`

### 蓝图事件
| 事件 | 类型 | 触发 |
|---|---|---|
| `OnAmmoChanged(Current, Reserve)` | BlueprintAssignable | 弹药任一变化（每端本机，OnRep 驱动）——**UI 弹药 HUD 订阅此事件刷新** |
| `OnReloadStart()` | BlueprintImplementableEvent | 换弹开始——蓝图播换弹动画/音效 |
| `OnReloadFinish()` | BlueprintImplementableEvent | 换弹完成——蓝图复位换弹动画 |

### 数据流
```
开火 (HandleFire, 服务端) → --CurrentAmmo → OnRep_Ammo (服务端本机) + 复制到客户端 OnRep_Ammo
  CurrentAmmo<=0 → StopFire（不空响、不自动换弹）

按 R (AFPSCharacter::Reload, 本地) → CanReload 预判 → ServerReload RPC
  → AFPSWeapon::ServerBeginReload → StopFire + bIsReloading=true → ReloadTimer(ReloadTime)
    → FinishReload：min(MagSize-Current, Reserve) 补入弹夹，bIsReloading=false

复活 / 首次装备 → AFPSCharacter 调 CurrentWeapon->ServerResetAmmo()（满夹 + 初始备弹）
```

### 关键设计
- **`StartFire` 守卫**：`bIsReloading || CurrentAmmo<=0` 时直接 return，不启 timer
- **服务端本机手动调 OnRep**：listen server host 不走复制，每处写弹药后手动调 `OnRep_Ammo()/OnRep_IsReloading()` 保证 host 自己的 UI/动画即时更新
- **换弹无空响惩罚**：弹夹空只停火，玩家自行按 R；不做自动换弹
...
`UFPSItemDef` 基类两个 virtual：`ServerCanUse(User, Entry)` / `ServerUseItem(User, Entry)`。每种道具一个子类 override。背包组件只调 `ItemDef->ServerUseItem`，**不关心细节**。

- 已实现 `UFPSHealItemDef`（血包）：`ServerCanUse` = 耐久>0 且 未满血；`ServerUseItem` = 回 `min(缺血, 耐久)` 血、扣等量耐久。
- `ServerUseItem` 直接改传入的 `Entry`（扣耐久/数量），背包组件在调用后检查：`bUsesDurability ? Durability<=0 : Count<=0` 为真则删格。**职责分离**：道具管效果，组件管删格。

> **升级路线（方案 B：Effect 列表，数据驱动）**：未来若需要"一个道具组合多个效果"（如"立即回50血 + 持续每秒回10血5秒 + 修10点护甲"），把 `UFPSItemDef` 改为持有 `TArray<UFPSItemEffect*>`，`ServerUseItem` 内部遍历 `Effect->Apply(User)`。`UFPSItemEffect` 子类：`UInstantHealEffect` / `UDurationHealEffect`（注册定时器在 Character 上每秒回血）/ `UArmorRepairEffect`。**关键**：背包组件 / Pickup / UI 全都只认 `ItemDef->ServerUseItem` 这个接口，升级时它们一行都不用改。持续效果（HoT/护盾）建议做成 Character 上的 `UBuffComponent` 持有"激活中的 Effect 实例 + 剩余时间"，服务端 Tick 推进、复制给客户端做 UI。
...
`CurrentPickupTarget` 是**本地状态不复制**（overlap 在每端各自维护）。`AFPSPickup` 沿用 `AFPSWeapon` 的"C++ 空 Root，蓝图加 Mesh"约定。
...
| UFPSItemDef | `DisplayName / Icon / bUsesDurability / MaxStack / DefaultValue / GridWidth / GridHeight / DropPickupClass` | 蓝图数据资产填的字段 |
| AFPSPickup | `GetItemDef()` / `OnPickupBlocked()` | 拾取提示文字 / 满包提示 |

### 蓝图接入步骤（血包 MVP）

> C++ 编译通过后在 UE 编辑器中操作。新 C++ 类无需手动"创建"，编译后自动出现在编辑器。

1. **数据资产 `DA_HealLarge`**：右键 Content → Miscellaneous → Data Asset（中文编辑器：**其他 → 数据资产**）→ 弹"选取数据资产类"→ 搜 `FPSHealItemDef` 选中。填 `DisplayName="大血包"` / `Icon` / `DefaultValue=64`（初始耐久）/ `DropPickupClass=BP_Pickup_HealLarge`（下一步建完回填，用于丢弃）。
   > 基类 `UFPSItemDef` 是 `Abstract` 不可选，**只能选子类 `FPSHealItemDef`**——这是预期。若"数据资产"对话框里搜不到，是没编译/热重载没生效，关编辑器全量编译重开。
2. **Pickup 蓝图 `BP_Pickup_HealLarge`**：继承 `AFPSPickup`，在 `DefaultRoot` 下加 StaticMesh（药包模型，**不要替换 root**），`ItemDef` 指向 `DA_HealLarge`，可调 `PickupRadius`。建完回到 `DA_HealLarge` 把 `DropPickupClass` 指向它（丢弃才能生成对应 Pickup）。
...
6. **手摆测试**：把 `BP_Pickup_HealLarge` 拖进 TestMap → 受伤后走近按 F → E 开背包右键格子选使用/丢弃 → 验证回血/扣耐久/耗尽删格、丢弃落地保留耐久全端同步。
...
| ServerUseInventoryItem | Client→Server | Reliable | 使用背包第 N 格道具（服务端权威执行 ItemDef 效果） |
