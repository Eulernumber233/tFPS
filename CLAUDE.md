...
## 物品生成系统（AFPSPickupSpawner）

场景手摆的生成点 Actor，服务端权威周期性生成 Pickup。**事件驱动**：生成一个 Pickup → 等待被拾取（销毁）→ 倒计时 → 生成下一个。

### 关键字段

| 字段 | 默认值 | 说明 |
|---|---|---|
| `SpawnTable` | 空数组 | 可生成的道具条目列表（ItemDef + BaseWeight + 可选 OverridePickupClass） |
| `DefaultPickupClass` | 空 | 默认 Pickup 蓝图（条目未指定时的兜底） |
| `BaseSpawnInterval` | 30s | 基础生成间隔（拾取后等待此时间 +- 随机偏移再生成） |
| `SpawnIntervalRandomDeviation` | 10s | 间隔随机偏移，最终 = BaseSpawnInterval + RandRange(-Dev, +Dev)，下限 1s |
| `InitialSpawnDelay` | -1 | 首次生成的固定延迟（秒）。>=0 直接用此值；<0 用常规间隔 |
| `BaseSpawnTier` | 1 | 游戏开始时可生成的最低 Tier |
| `MaxSpawnTier` | 5 | TierRampTime 后完全解锁的最高 Tier |
| `TierRampTime` | 300s | Tier 从 Base 线性增长到 Max 的时长 |
| `BiasItemClass` | 空 | 偏向道具子类（该类条目权重 *= BiasDegree） |
| `BiasDegree` | 2.0 | 偏向倍率 |
| `SpawnCheckRadius` | 80cm | 防堆叠检查半径 |
| `bAutoStart` | true | BeginPlay 时自动启动生成循环 |

### 生成循环（事件驱动）

```
BeginPlay → StartSpawning()
  → 等 InitialSpawnDelay（或常规间隔）后首次 TrySpawn
  → SpawnActorDeferred + 设 ItemDef + FinishSpawning  ← 确保蓝图 Construction Script 能读到 ItemDef
  → OnSpawnSucceeded(Pickup)                          ← 蓝图事件：生成成功
  → 等待 Pickup 被拾取……
玩家拾取 → Pickup->Destroy() → OnDestroyed 委托
  → OnSpawnedPickupDestroyed
    → OnWaitingForRespawn()                           ← 蓝图事件：进入等待，开始倒计时
    → ScheduleNextSpawn()
  → 等 BaseSpawnInterval ± 随机偏移后 TrySpawn → 循环
```

- **不轮询**：生成成功后的 Pickup 通过 `OnDestroyed` 委托跟踪，被拾取才安排下一次生成。避免 timer 空转。
- **防堆叠**：`TrySpawn` 首检 `CurrentSpawnedPickup.IsValid()`（有则跳过，等委托触发），再检 `IsPickupAlreadyAtSpawnPoint()`（重叠测试兜底手摆 Pickup）。
- **`ForceSpawn()`**：跳过等待，立刻尝试生成（仍受防堆叠约束）。

### 蓝图事件

| 事件 | 触发时机 | 用途 |
|---|---|---|
| `OnSpawnSucceeded(AFPSPickup*)` | 生成成功 | 播放生成特效（粒子/音效） |
| `OnWaitingForRespawn()` | 上一轮 Pickup 被拾取，进入等待 | 显示倒计时/关闭特效 |

### 蓝图接入步骤

1. 创建继承 `AFPSPickupSpawner` 的蓝图子类（如 `BP_PickupSpawner_Ammo`）
2. 填写 `SpawnTable`：添加条目，每个条目选 ItemDef 数据资产 + 设权重
3. 填写 `DefaultPickupClass`：选 `BP_Pickup_Generic`（或条目的 OverridePickupClass 各自的专属蓝图）
4. 实现 `OnSpawnSucceeded`：播放生成特效
5. 实现 `OnWaitingForRespawn`：关闭特效/显示倒计时
6. 拖几个 Spawner 蓝图到 TestMap → 启动测试

> **注意**：SpawnTable 必须至少有一条有效条目（ItemDef 非空 + Tier <= CurrentMaxTier + BaseWeight > 0），且必须能解析出 PickupClass（条目 Override → Spawner Default → ItemDef->DropPickupClass），否则不会生成任何物品。
...
`CurrentPickupTarget` 是**本地状态不复制**（overlap 在每端各自维护）。`AFPSPickup` C++ 不提供可见网格体，由蓝图子类自行添加 StaticMesh 组件（读 `ItemDef->PickupMesh` 赋模型）。
...
2. **通用 Pickup 蓝图 `BP_Pickup_Generic`**：继承 `AFPSPickup`，在蓝图层添加 StaticMesh 组件挂在 DefaultRoot 下（读 `ItemDef->PickupMesh` 赋模型）。`ItemDef` 留空（由 Spawner/手摆时单独指定）。`DropPickupClass` 所有普通道具的 DA 都指向它。需要特殊拾取表现的道具才单独建 `BP_Pickup_xxx` 子类，此时 DA 的 `PickupMesh` 留空、`DropPickupClass` 指向专用子类。
...
| ~~自动刷新点~~ | ✅ 已实现 | `AFPSPickupSpawner` 详见下方"物品生成系统"章节
...
| ItemDef | FPSPickup | Replicated |
| bHasOverride | FPSPickup | Replicated |
| OverrideEntry | FPSPickup | Replicated |
